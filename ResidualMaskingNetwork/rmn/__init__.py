...
class RMN:
    def __init__(self, face_detector=True, device="cpu"):
        if face_detector is True:
            self.face_detector = get_ssd_face_detector()
        self.emo_model = get_emo_model().to(device)
        self.history = []
    ...
    def detect_emotion_for_single_face_image(self, face_image):
        ...
        def first_hook_fn(module, input, output):
            history["first"] = output.clone().detach()
        first_handel = self.emo_model.layer1.register_forward_hook(first_hook_fn)

        # save second - take output of self.emo_model.mask4.conv2.downsample
        def second_hook_fn(module, input, output):
            history["second"] = output.clone().detach()
        second_handel = self.emo_model.mask4.conv2.downsample.register_forward_hook(second_hook_fn)

        def third_and_forth_handler(module, input, output):
            history["third"] = input[0].clone().detach()
            history["forth"] = output.clone().detach()
        third_and_forth_handel = self.emo_model.fc.register_forward_hook(third_and_forth_handler)
        ...
        output = self.emo_model(face_image)
        proba = output # TODO maybe proba = torch.softmax(output, 0)
        ...
        #remove hooks
        first_handel.remove()
        second_handel.remove()
        third_and_forth_handel.remove()
    ...
    @torch.no_grad()
    def torch_detect_emotion_for_single_frame(self, frame):
        assert len(frame.shape) != 4 or frame.shape[0] == 1, "only support single frame"
        if len(frame.shape) == 4 and frame.shape[0] == 1:
            frame = frame.squeeze(0)

        img = (frame.repeat(3, 1, 1).permute(1, 2, 0).to("cpu").numpy()*255).astype(np.uint8)
        (
            emo_label,
            emo_proba,
            proba_list,
        ) = self.detect_emotion_for_single_face_image(img)
        converted = [
            proba_list[0]["angry"],
            proba_list[1]["disgust"],
            proba_list[2]["fear"],
            proba_list[3]["happy"],
            proba_list[4]["sad"],
            proba_list[5]["surprise"],
            proba_list[6]["neutral"],
        ]
