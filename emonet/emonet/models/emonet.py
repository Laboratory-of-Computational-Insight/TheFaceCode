...
class EmoNet(nn.Module):
    ...
    def forward(self, x, reset_smoothing=False, mode=0):
        if mode==0:
            return self.forward_new(x, reset_smoothing)
        return self.forward_old(x, reset_smoothing)

    def forward_new(self, x, reset_smoothing=False):
        image_size = 256
        trans = torchvision.transforms.Resize(size=(image_size, image_size))
        img = trans(x)
        img = img.repeat(1, 3, 1, 1)

        results = self.forward_old(img, reset_smoothing)
        ...
        arr = results["expression"]
        # arr = arr.squeeze(0)
        # arr = arr[:, [6,5,1,2,3,0,5,7]]
        return arr

    def forward_old(self, x, reset_smoothing=False):
        history = {}
        ...
        # history['bn1']=x.clone().detach().to("cpu")
        ...
        # history["emo_feat"] = emo_feat.to("cpu").clone().detach()

        emo_feat_conv1D = self.conv1x1_input_emo_2(emo_feat.to("cuda"))
        ...
        def emo_fc_2_hooks(self, input, output):
            history["emo_fc_2"] = output.clone().detach().to("cpu")
        handle = self.emo_fc_2[1].register_forward_hook(emo_fc_2_hooks)
        ...
        history["final"] = final_features.clone().detach().to("cpu")
